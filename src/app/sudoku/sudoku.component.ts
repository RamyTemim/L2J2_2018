...
export class SudokuComponent implements OnInit {
  ...
  validateGrid() {
    ...
    let success = (response) => {
      if (response === 'true') {
        alert('Félicitation, vous avez réussi à finir la grille !!!!!!');
      }
      else alert("Désolé, la grille est incorrecte -> vous n'avez pas trouvé la solution");
    };

    let valideGrid = SudokuComponent.checkGridValues();
    if (!valideGrid) {
      alert("Désolé, la grille n'est pas complète -> elle ne peut être validée que lorqu'elle entièrement remplie");
      return;
    }
